...
def main(args):
    # filename
    dataset_type = 'monte_carlo_[-9.9, 15.3, 40.5]' + '_' + str(
        args.signal_range)  # random_input_sep_1,monte_carlo_[10.1, 13.3],equal_sep_2,several_sep
    ...
    # theta_set = Create_monte_carlo_theta(all_theta=[np.array([10.1, 35.3, 60.5])], repeat_num=5000)
    theta_set = Create_monte_carlo_theta(all_theta=[np.array([-9.9, 15.3, 40.5])], repeat_num=5000)
    ...
    for repeat_i in range(args.repeats):
        for step_1, snap in enumerate(args.snaps):
            ...
            Create_datasets(dataset, args.k, theta_set, batch_size=100, snap=snap, snr=args.snr, snr_set=0)
...
if __name__ == "__main__":
    ...
    load_root = os.path.join(root_path, 'results', 'vit', f'vit_M_8_k_3_v_snap')
