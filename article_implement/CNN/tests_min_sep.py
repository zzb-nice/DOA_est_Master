...
def main(args):
    # filename
    dataset_type = 'equal_sep_rho2' + '_' + str(
        args.signal_range)
    ...
    for repeat_i in range(args.repeats):
        for step_1, sep in enumerate(args.intervals):
            ...
            dataloader = file_array_Dataloader(load_file, 256, False, load_style='torch', input_type='enhance_scm', output_type='doa')
            ...
            suc_ratio, test_loss, predict_result = test(model, dataloader, loss_function, args.device, True, args.k)
...
if __name__ == "__main__":
    ...
    dataset_path = os.path.join(root_path, 'data', 'ULA_data', 'test', f'M_8_k_3_snap_10_snr_5_rho2_min_sep')
    load_root = os.path.join(root_path, 'results', 'CNN_load_path', f'std_CNN_M_8_k_3')
