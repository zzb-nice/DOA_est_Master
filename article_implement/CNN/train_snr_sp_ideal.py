...
def main(args):
    # filename
    dataset_type = 'random_input_sep_3_repeat' + '_' + str(
        args.signal_range)  # several_seps[1, 2, 3, 4, 8, 12, 16, 20, 24, 30, 45, 80]
    ...
    # loss function
    loss_function_valid = torch.nn.MSELoss()
    # loss_function = torch.nn.CrossEntropyLoss()
    # 改了MultiLabelSoftMarginLoss source code
    ...
    for step_1, snr in enumerate(args.snrs):
        ...
        train_dataloader = array_Dataloader(dataset, 256, load_style='torch',
                                            input_type='enhance_truth_scm', output_type='spatial_sp')
        val_dataloader = array_Dataloader(val_dataset, 256, shuffle=False, load_style='torch',
                                          input_type='enhance_truth_scm', output_type='spatial_sp')
...
if __name__ == "__main__":
    ...
    root_path = '../../'
    root = os.path.join(root_path, 'results', 'CNN_load_path', 'std_CNN_M_16_k_2_ideal')
