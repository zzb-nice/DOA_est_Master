...
from model import ModelGenerator,SubspaceNet
...
@torch.no_grad()
def test_subspace(model, data_loader, loss_function, device, grid_to_theta=True, k=3, return_sp=False):
    model.eval()
    ...
    for step, data in enumerate(data_loader):
        ...
        pred, doa_all_predictions, roots, Rz = model(input.to(device))
        if grid_to_theta:
            pred = F.sigmoid(pred)
    ...
    if grid_to_theta:
        ...
        print(f"success ratio = {np.sum(succ_vec)/len(succ_vec)}")
        if return_sp:
            sp_result = torch.concatenate(sp_result, 0).cpu().numpy()
            return np.sqrt(accu_loss.item() / (step + 1)), pred_result, sp_result
...
def main(args):
    ...
    for repeat_i in range(args.repeats):
        for step_1, snr in enumerate(args.snrs):
            ...
            dataloader = file_array_Dataloader(load_file, 256, False, load_style='torch', input_type='tau_cat_scm', output_type='doa')
            ...
            test_loss, predict_result = test_subspace(model, dataloader, loss_function, args.device, args.grid_to_theta, args.k)
...
if __name__ == "__main__":
    ...
    dataset_path = os.path.join(root_path, 'data', 'ULA_data', 'test', f'M_16_k_3_test1_rho0')
    load_root = os.path.join(root_path, 'article_implement', 'SubspaceNet', 'results', f'SubspaceNet_M_16_k_3_snap_10')
