...
@torch.no_grad()
def test_asl(model, data_loader, loss_function, device, asl_start_angle, grid_to_theta=True, k=3):
    model.eval()
    ...
    # else:
    return np.sqrt(accu_loss.item() / (step + 1)), pred_result


def main(args):
    test_type = "random_input"  # random_input,monte_carlo[10,20,30],monte_carlo[10,13,16]
    # filename
    dataset_type = f'file_test1_{test_type}_rho0'  # save path
    ...
    # our model
    model = MLP(args.model_size, args.drop_out_ratio, sp_mode=True, start_angle=args.signal_range[0],
                end_angle=args.signal_range[1], step=args.step_used)
    model_name = 'ASL'
    ...
    predict_results = np.zeros((len(args.snrs), num_samples, args.k-1))
    ...
    for repeat_i in range(args.repeats):
        for step_1, snr in enumerate(args.snrs):
            ...
            dataloader = file_array_Dataloader(load_file, 256, False, 'torch', 'scm_vec', 'sep_k')
            ...
            test_loss, predict_result = test_asl(model, dataloader, loss_function, args.device, args.signal_range[0],
                                                 args.grid_to_theta, args.k-1)
            ...
            savemat(os.path.join(save_path, f'construct_snap_{args.snap}_snr_{snr}.mat'),
                    {'R': np.stack(dataset.ori_scm, 0), 'true_sep': np.stack(dataset.sep_k),
                     'pred_sep': predict_result, 'true_doa': np.stack(dataset.doa),
                     'grid': dataset.signal_grid, 'steer_vec': dataset.get_A(dataset.signal_grid, None)})
...
if __name__ == "__main__":
    ...
    dataset_path = os.path.join(root_path, 'data', 'ULA_data', 'test', f'M_8_k_3_test1_rho0')
    load_root = os.path.join(root_path, 'article_implement', 'ASL', 'results', f'ASL_M_8_k_3')
    ...
    # model parameters
    # ...
    parser.add_argument('--model_size', type=list, default=[56, 512, 512, 512, 121])
    parser.add_argument('--grid_to_theta', type=bool, default=True)
    parser.add_argument('--drop_out_ratio', type=float, default=0)
