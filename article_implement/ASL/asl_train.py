...
def main(args):
    ...
    for step_1, snr in enumerate(args.snrs):
        ...
        Create_datasets(dataset, args.k, train_theta_set, batch_size=512, snap=args.snap, snr=snr)
        Create_datasets(val_dataset, args.k, val_theta_set, batch_size=512, snap=args.snap, snr=snr)

        train_dataloader = array_Dataloader(dataset, 1024, load_style='torch', input_type='scm_vec',
                                            output_type='sep_k_spatial')
        val_dataloader = array_Dataloader(val_dataset, 1024, shuffle=False, load_style='torch',
                                          input_type='scm_vec', output_type='sep_k_spatial')
...
if __name__ == "__main__":
    ...
    root = os.path.join(root_path, 'results', 'ASL_M_8_k_3')
