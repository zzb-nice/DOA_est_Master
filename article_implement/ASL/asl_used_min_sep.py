...
def main(args):
    # filename
    dataset_type = f'equal_sep_snr_{args.snr}_rho{args.rho}' + '_' + str(
        args.signal_range)
    ...
    predict_results = np.zeros((len(args.intervals), max_len, args.k-1))
    ...
    for repeat_i in range(args.repeats):
        for step_1, sep in enumerate(args.intervals):
            ...
            savemat(os.path.join(save_path, f'construct_snap_{args.snap}_snr_{args.snr}_sep_{sep}.mat'),
