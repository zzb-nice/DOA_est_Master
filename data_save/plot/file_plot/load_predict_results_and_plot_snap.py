...
def main(args):
    ...
    load_file = os.path.join(args.dataset_path, f'test_{test_type}_snap_10.npz')
    ...
    load_path = os.path.join(args.root_path,
                             f'results/vit/vit_v_snap/{args.file_test_id}_{test_type}/model_predict_results.npy')
    # 读取预测结果
    model_pred = np.load(load_path)
    assert model_pred.shape[0] == len(args.snaps), 'error input size'

    # 读取各个模型的预测结果
    contrast_model_name = ['MUSIC', 'Unity-ESPRIT', r'$\ell_1$-SVD', 'SPE-CNN', r'Learning-SPICE']
    load_root = [
        os.path.join(args.root_path,
                     f'results/vit/vit_v_snap/file_test1_{test_type}/MUSIC_predict_results.npy'),
        os.path.join(args.root_path,
                     f'results/vit/vit_v_snap/file_test1_{test_type}/Unity-ESPRIT_predict_results.npy'),
        os.path.join(args.root_path,
                     f'results/vit/vit_v_snap/file_test1_{test_type}/$\ell_1$-SVD_predict_results.npy'),
        os.path.join(args.root_path,
                     f'results/vit/vit_v_snap/file_test1_{test_type}/SPE-CNN_predict_results.npy'),
        os.path.join(args.root_path,
                     f'article_implement/Learning_SPICE/results/R_fit_snap_M_8_k_3/file_test1_{test_type}/pred_results_and_RMSE.mat')
    ]
    ...
    # 计算并保存模型的RMSE
    model_loss = np.zeros(len(args.snaps))
    model_succ_ratio = np.zeros(len(args.snaps))
    for i in range(len(args.snaps)):
    ...
    # 打印每个快拍数下的成功率
    print("\nSuccess ratios for each snapshot:")
    ...
    # 绘制loss对比图
    loss_1d_v_plot(model_loss, model_name, args.snaps, 'Snapshots', contrast_model_loss, False,
...
if __name__ == "__main__":
    ...
    test_type = 'monte_carlo[10.1,35.3,59.5]'  # random_input,monte_carlo[-9.9,15.3,39.5],monte_carlo[5.1,30.3,54.5]
    ...
    args.save_root = os.path.join(root_path, 'data_save', 'vit', 'v_snap',
                                  f'{args.file_test_id}_{args.test_type}')
