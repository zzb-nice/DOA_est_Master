...
from vit_tranfer_learning.theta_creater.theta_creater import same_data_Creater_2d
...
def evaluate(model, data_loader, loss_function, device, epoch, grid_to_theta=True, k=3):
    ...
    for step, data in enumerate(data_loader):
        input, labels = data
        labels = torch.flatten(labels, start_dim=1)  # 展开,内维度优先
...
def main(args):
    ...
    train_theta_set = Create_random_k_input_theta(args.k, args.theta_range[0], args.theta_range[1], args.phi_range[0],
                                                  args.phi_range[1], 500, min_delta_theta=3)
    val_theta_set = Create_random_k_input_theta(args.k, args.theta_range[0], args.theta_range[1], args.phi_range[0],
                                                args.phi_range[1], 20000, min_delta_theta=3)
    ...
    for step_1, snr in enumerate(args.snrs):
        ...
        dataset, val_dataset = UCA_dataset(args.M, -90, 90, 1, args.rho), UCA_dataset(args.M, -90, 90, 1, args.rho)
        base_dataset = UCA_dataset(args.M, -90, 90, 1, args.ori_rho)
        Create_datasets(dataset, args.k, train_theta_set, batch_size=50, snap=args.snap, snr=snr)
        Create_datasets(val_dataset, args.k, val_theta_set, batch_size=512, snap=args.snap, snr=snr)
        data_creater = same_data_Creater_2d(base_dataset, 'scm')
...
if __name__ == "__main__":
    ...
    root = os.path.join(root_path, 'results', 'vit', f'vit_2D_DOA_origin')
