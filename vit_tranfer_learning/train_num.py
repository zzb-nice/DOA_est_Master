...
def main(args):
    ...
    # loss function
    loss_function_evaluate = torch.nn.MSELoss()
    ...
    for step_1, train_sample_num in enumerate(args.train_sample_nums):
        ...
        train_theta_set = Create_random_k_input_theta(args.k, args.signal_range[0],
                                                      args.signal_range[1], train_sample_num, min_delta_theta=2)
        ...
        for epoch in range(args.epochs):
            train_loss = train_one_epoch(model, train_dataloader, loss_function, optimizer, args.device, epoch + 1,
                                         False, args.k)
            val_loss = evaluate(model, val_dataloader, loss_function_evaluate, args.device, epoch + 1, False, args.k)
...
if __name__ == "__main__":
    ...
    root = os.path.join(root_path, 'results', 'vit', f'vit_range_num_rho{rho}')
