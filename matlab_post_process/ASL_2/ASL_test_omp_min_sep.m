...
if length(threshold_vec) ~= length(seps) 
...
load_root = '/home/xd/DOA_code/open_source_code/article_implement/ASL/results/ASL_M_8_k_3/equal_sep_snr_0_rho1_(-60, 60)/';
file_name = ['construct_snap_' num2str(snap) '_snr_' num2str(snr) '_sep_' '[[' num2str(seps(1)) ', ' num2str(seps(1)) ']]' '.mat'];
...
for ii=1:length(threshold_vec)
    noise_amp = 1*10^(-snr/20);
    threshold  = threshold_vec(ii);

    % load data
    file_name = ['construct_snap_' num2str(snap) '_snr_' num2str(snr) '_sep_' '[[' num2str(seps(ii)) ', ' num2str(seps(ii)) ']]' '.mat'];
    load_file = [load_root file_name];   % load 之后不用生成信号
    data = load(load_file);
...
plot(seps, RMSE_RMUSIC ,'-o', 'LineWidth', 2);
hold on;
plot(seps, RMSE_R_cons,'-o', 'LineWidth', 2);
hold off;
grid on;
