function [succ, ang_est, Ps] = ASL_R_construct_k_3(R, ULA_N, threshold, THETA_angles, sep, noise_power)
% INPUTS:
...
assert(length(sep)==2,'wrong input shape')
THETA_angles_2 = THETA_angles + sep(1);
THETA_angles_3 = THETA_angles + sep(2);

A_dic = zeros(ULA_N * ULA_N, NGrids);
A_dic_2 = zeros(ULA_N * ULA_N, NGrids);
A_dic_3 = zeros(ULA_N * ULA_N, NGrids);
for n = 1:NGrids
    ...
    ULA_gridn = ULA_steer_vec(THETA_angles_3(n), ULA_N);
    A_dic_mtx = kron(ULA_gridn, ULA_gridn');
    A_dic_3(:, n) = A_dic_mtx(:);
end

% Combine dictionaries
A_combined = A_dic + A_dic_2 + A_dic_3;
Rdr = R(:);
em = ones(ULA_N);
em = noise_power * em(:);
