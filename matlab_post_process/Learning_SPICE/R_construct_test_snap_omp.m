...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
% change SNR
snap_vec = [1,5,10,30,50,100]; % number of snapshots
snr = -10; % SNR values
threshold_vec = [1,1,1,1,1,1];  % make sure the success ratio>90%
if length(threshold_vec) ~= length(snap_vec)
    error('Error: The lengths of the two vectors are not equal. The program is terminated.')
end

% load data to known N_sim
load_root = '/home/xd/DOA_code/open_source_code/article_implement/Learning_SPICE/results/R_fit_snap_M_8_k_3/file_test1_monte_carlo[5.1,30.3,54.5]/';
file_name = ['R_ext_' num2str(snap_vec(1)) '_snr_' num2str(snr) '.mat'];
...
for ii=1:length(threshold_vec)
    ...
    % load data
    file_name = ['R_ext_' num2str(snap_vec(ii)) '_snr_' num2str(snr) '.mat'];
...
plot(snap_vec, RMSE_RMUSIC ,'-o', 'LineWidth', 2);
hold on;
plot(snap_vec, RMSE_R_cons,'-o', 'LineWidth', 2);
hold off;
grid on;
